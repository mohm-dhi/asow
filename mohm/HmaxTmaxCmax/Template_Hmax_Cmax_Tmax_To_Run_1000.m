...
EVAOpt.plotflag         = 1;                                           % 2
EVAOpt.EVadju           = 0;
EVAOpt.T                = [1, 5, 10, 25, 50, 100, 500, 1000];	       % Return period for second run [1, 10,  50, 100];
...
if f==2
    ...
    Time                 = tmp.Time;
    MWD                  = tmp.Values(:,6);
    D_SW                 = m_structure(Name,xyz,ttt_SW,legend_SW,[Time MWD ],'MWD_Total', 1,bins_D);
    H_SW.ascii           = '.xlsx';
    T_SW.ascii           = '.xlsx';
    D_SW.ascii           = '.xlsx';
