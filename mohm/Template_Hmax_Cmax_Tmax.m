...
output_directory = 'C:\mohm\Hmax_Tmax_Cmax\';
...
% SP_file = '\\USDEN1-STOR.DHI.DK\Projects\41806529\03_PROCESSED_MOOD_data\02-OutputLocations\SpectralAnalysis\P8_Waveage_Wave_energy_density_SW_{US-EC}_(1979-01-15_2021-12-31).mat';
...
dt_SW      = hours(diff(datetime(datestr(Time_SW(1:2)))))*60;
ttt_SW     = [datenum([y0 m0 d0]) datenum([y1 m1 d1]) dt_SW dt_SW];
dt_HD      = hours(diff(datetime(datestr(Time_HD(1:2)))))*60;
...
[C,IA,IB]  = intersect(datetime(datestr(Time_SW)),datetime(datestr(Time_HD)));
Time_HD    = Time_HD(IB);
Time_SW    = Time_SW(IA);
...
%     X = M;
    ...
    ZC_file              = 'C:\mohm\Hmax_Tmax_Cmax\Tmax_all\ASOW2_Zerocrossing_ED2f_deg_SW_{US-EC}_(1979-01-01_2021-12-31).mat';
    ZC_file              = 'C:\mohm\Hmax_Tmax_Cmax\ASOW2\Tmax\ASOW2_Zerocrossing_ED2f_deg_SW_{US-EC}_(1979-01-01_2021-12-31).mat';
    
    
    % Run m_zerocrossing
    root_dir             = pwd;
    cd(out_pth_Tmax)
    if exist(ZC_file, 'file') ~= 2
...
for j=1:length(H_SW.time)
    ...
    root_dir             = pwd;
    cd(out_pth_Tmax)
    H_SW.ttt_str = replace(H_SW.ttt_str, '_', ' _ ');
